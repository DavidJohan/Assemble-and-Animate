...
int dxl_initialize()
{
	...
	dxl_set_baud( DEFAULT_BAUDNUMBER );
...
int dxl_get_baud()
{
	return (int)gbBaudNum;
}

void dxl_set_baud( int baudnum )
{
	float baudrate;

	if( gbBaudNum == baudnum )
		return;
...
void dxl_tx_packet()
{
	...
	if( gbInstructionPacket[INSTRUCTION] == INST_READ )
		dxl_hal_set_timeout( gbInstructionPacket[PARAMETER+1] + 100 ); //changed from 6 to solve timeout error
	else
		dxl_hal_set_timeout( 100 ); //changed from 6 to solve timeout error
...
void dxl_rx_packet()
{
	...
	if( gbCommStatus == COMM_TXSUCCESS )
	{
		...
		if( gbInstructionPacket[INSTRUCTION] == INST_READ )
		{
			gbRxPacketLength += gbInstructionPacket[PARAMETER+1];
		}
	}

	nRead = dxl_hal_rx( (unsigned char*)&gbStatusPacket[gbRxGetLength], gbRxPacketLength );
	gbRxGetLength += nRead;
	gbRxPacketLength -= nRead;
	if( gbRxPacketLength > 0 )
	{
	...
	for( i=0; i<(gbRxGetLength-1); i++ )
	{
		...
		{
			gbRxPacketLength = gbRxGetLength-1;
			break;
		}
	}
	if( gbRxPacketLength > 0 )
	{
		for( i=0; i<(gbRxGetLength-gbRxPacketLength); i++ )
			gbStatusPacket[i] = gbStatusPacket[i+gbRxPacketLength];

		gbRxGetLength -= gbRxPacketLength;
		gbCommStatus = COMM_RXWAITING;
		return;
	}
	...
	if( gbInstructionPacket[ID] != gbStatusPacket[ID]
		|| gbStatusPacket[LENGTH] > (MAXNUM_RXPARAM+2)
		|| gbStatusPacket[gbStatusPacket[LENGTH]+3] != checksum )
	{
