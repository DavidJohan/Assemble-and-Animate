...
void applyRotation(int actuator) {
	...
	if((!rotations[actuator].pause) && (!rotations[actuator].pausePeriode)) {
	...
	else if(rotations[actuator].pausePeriode) // This pice of code stops the gait for one periode only.
	{
		float periodeTime = 1.0f/rotations[actuator].frequency;
		if(t-rotations[actuator].pauseStart > periodeTime) {
			rotations[actuator].pausePeriode = false;
		}
		dynamixelApi_wheelMove(actuator, 0, rotations[actuator].dir);
	}
	else if(rotations[actuator].pause) //This stops the gait as long as the "pause" is given to the motors
	{
		dynamixelApi_wheelMove(actuator, 0, rotations[actuator].dir);
	}
...
void gait_selection()
{
	...
	float offset_LF = -2.17;
	float offset_LH = -2.17;
	float offset_RF = 0;
	float offset_RH = 0;
